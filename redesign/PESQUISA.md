...
| **DeepSeek Harness (`dsh`)** | `0.1.0-rc.5`, Node 24, micro-kernel Cordis, seams: models/tools/skills/sessions/sandboxes/storage/loops/scheduling/UI. "THERE WILL BE COMPATIBILITY-BREAKING CHANGES". Session log append-only nativo. | alternativa dormente ao LangGraph | preview instável — adapter DORMENTE escrito em **P4-06** (`redesign/grafo/adapters/dsh.md` + `dsh.py` `ENABLED=False`). **Gatilho de reavaliação:** tag estável do `dsh` (≥1.0 / fim do aviso de breaking changes) **E** motivo concreto pós-Fase 8. |
