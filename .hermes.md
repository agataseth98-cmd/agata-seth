...
**Estado:** duas invocações reais tentadas 17/08/2026, ainda sem parecer válido. 1ª: HTTP 429 nas duas tentativas permitidas. 2ª: sucesso técnico, mas o modelo gastou o teto de saída inteiro (8.000 tokens) tentando CALCULAR um hash SHA256 de cabeça em `reasoning_content`, nunca produziu o parecer — achado real, corrigido: `thinking: {"type": "disabled"}` adicionado à chamada (parâmetro confirmado em `docs.z.ai/api-reference`), `TETO_TOKENS_SAIDA` 8.000→4.000. Reenvio único com o formato junto (mecanismo de REGRAS "Segunda opinião") bateu em 429 de novo, duas tentativas. Mecanismo funciona como desenhado nos dois eixos — para sozinho, não inventa, não deixa lixo. Provedor instável neste dia. Ver MEMÓRIAS (206)-(208), (211), (212).
