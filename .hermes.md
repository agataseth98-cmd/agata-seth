...
- **[PROPOSTO, não habilitado] P-7, checagem de citação.** `scripts/checar_citacao.sh`, testado positivo/negativo isolado e contra o corpus real (5 citações no formato `(n - síntese)` existentes hoje, 1 suspeita — exemplo ilustrativo dentro de uma ordem citada em (162), não erro de conteúdo). Fecha a lacuna que a expedição RLM achou (fabricação por citação errada, não por entrada inexistente). Fora do `pre-commit` até o Humano decidir habilitar. MEMÓRIAS (203).
