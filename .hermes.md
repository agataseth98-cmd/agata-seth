...
**Condição 2 (MEMÓRIAS (206)), ordem obrigatória da chave:** obter a chave → confirmar o formato real dela → testar P-1 com uma chave FALSA do mesmo formato → confirmar que alarma → só então guardar a chave real em `~/.hermes/.env` (`ZHIPU_API_KEY=`). **Testado com 4 formatos plausíveis (MEMÓRIAS (207))** (hex 32, `id.secret` com ponto, UUID com hífen, prefixado `sk-...`) contra o padrão genérico já existente em P-1 — os 4 alarmaram. **Não é o teste final:** o formato real da chave Zhipu não está confirmado em documentação pública; o teste definitivo espera a chave real (ou pelo menos a forma dela) chegar.
...
**Estado:** script escrito, testado ponta a ponta com resposta simulada (guardas de conteúdo privado/tamanho/chave ausente, checagem de formato do parecer, escrita do arquivo — todos confirmados, nenhum contra a API real). **Bloqueado:** chave real — exige conta Zhipu, que este executor não pode criar (sem navegador/e-mail/pagamento). Ver MEMÓRIAS (206), (207).
