...
**Verificação mecânica, não só descrição:** `scripts/perimetro.sh` P-2 audita sudoers, mas é **estruturalmente SKIP** sempre que roda sem `sudo -n` não-interativo — caso normal do hook, o próprio controle funcionando, nunca contado como OK. Fechamento pleno exige contexto root: ALTERAÇÃO DE SISTEMA, proposta ao Humano, não implementada. MEMÓRIAS (193).
