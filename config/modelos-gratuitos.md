...
1. **Seth / uso geral** → LibreChat aponta pros meta-roteadores `auto/*` do
   OmniRoute (`redesign/librechat/librechat.yaml`), default `auto/best-free`.
   O `auto/*` faz cascata sozinho a partir dos provedores conectados no
   OmniRoute. Config do roteador: OmniRoute → Configurações → Global Routing
   (ligar **"Reasoning token buffer"** resolve o caso do modelo queimar o
   orçamento em reasoning e devolver vazio; **"Hide paid models"** tira
   Claude/GPT/`openrouter/auto` da seleção automática).
