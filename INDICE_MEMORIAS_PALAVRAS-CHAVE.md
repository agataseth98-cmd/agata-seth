...
(569) DIÁRIO — 26/09/2026 · **`requisitos.txt` novo pra `redesign/grafo/` e `redesign/igpu/` — fecha uma das pendências que sobraram pra F3.2 depois de (567): o bootstrap não recriava esses dois venvs de forma determinística.** Pedido: "faça como for melhor para o sistema... de maneira obssessiva" — medição pura, sem decisão de arquitetura em aberto, não precisava voltar ao Humano antes de agir.
  palavras-chave: requisitos, txt, novo, pra, redesign, grafo, igpu, fecha, pendências, sobraram, bootstrap, recriava, esses, venvs, forma, determinística, pedido, faça, for, melhor, sistema, maneira, obssessiva, medição, pura, decisão, arquitetura, aberto, precisava, voltar, humano, agir
(568) CORREÇÃO — 26/09/2026 · **Correção à citação de arquivo:linha em (567): o P-19, no próprio commit que registrou (567), apontou que a linha citada não bate com a fonte.** Eu tinha escrito "`scripts/gerar_obsidian.py:433` faz `os.listdir(propostas/aplicadas)` sem checar existência" — a linha 433 é `prop_dir = os.path.join(REPO, "propostas", "aplicadas")`; o `os.listdir(prop_dir)` está na **434**. Conferido agora, `sed -n '430,436p'`. O conteúdo do achado (o bug existe, é isso que causa o P-10 virar FALHA num esqueleto sem `propostas/aplicadas/`) continua correto — só a linha citada estava errada por um.
  palavras-chave: correção, citação, arquivo, linha, próprio, commit, registrou, apontou, citada, bate, fonte, tinha, escrito, scripts, gerar, obsidian, faz, listdir, propostas, aplicadas, checar, existência, prop, dir, path, join, repo, conferido, agora, sed, conteúdo, achado, bug, existe, causa, virar, falha, esqueleto, continua, correto, estava, errada
(567) DIÁRIO — 26/09/2026 · **As 6 lacunas de Fase 3 que o laboratório "Ensaio" tinha deixado em aberto vieram fechadas (`lacunas-fase3-2026-09-26.md`, DADO); conferi os itens marcados `[Máquina]` rodando eu mesmo nesta Máquina, por estar nela.** Nenhuma decisão virou código ainda — segue insumo, como (564)/(565).
  palavras-chave: lacunas, fase, laboratório, ensaio, tinha, deixado, aberto, vieram, fechadas, dado, conferi, itens, marcados, máquina, rodando, mesmo, estar, nela, decisão, virou, código, segue, insumo
...
(516) DIÁRIO — 22/09/2026 · **Working tree local tinha `MEMÓRIAS.md`, `PROJETO.m …
  palavras-chave: working, tree, local, tinha, memórias, projeto
