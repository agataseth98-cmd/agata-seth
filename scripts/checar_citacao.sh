#!/usr/bin/env bash
# P-7 (proposto, MEMÓRIAS (202)/(203)) -- checagem de citação. Decisão do
...
#
# AINDA NÃO ESTÁ NO pre-commit. Ordem do Humano: rodar contra as ~197
# entradas existentes, reportar a taxa de falso positivo, só DEPOIS
# propor habilitar -- não decidir aqui. Ver MEMÓRIAS (203).
#
...
# não o canon (ordem do Humano, doutrina de defesa proporcional).
set -uo pipefail
...
citacoes = padrao_citacao.findall(texto)

total = 0
suspeitos = 0
for n_str, sintese in citacoes:
    total += 1
    n = int(n_str)
...
print(f"__RESUMO_P7__ total_citacoes={total} suspeitos={suspeitos}")
