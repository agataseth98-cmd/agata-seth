...
Lê SOMENTE três arquivos, no topo do repositório: REGRAS.md, PROJETO.md,
MEMÓRIAS.md. NUNCA lê de memoria/missoes/ -- nem a esfera pessoal
(segunda-camada), nem a de projeto (agata-sistema). A garantia é o conjunto
de entrada fixo abaixo (FONTES) e a reconstrução byte a byte no fim.
...
do repo principal):
  - indice.md    -- Opção A de MEMÓRIAS (296)/(298):
                    ...
                    Parte 3: só as linhas de título das entradas de MEMÓRIAS
                             (nº + tipo + data + título), mais recente primeiro,
                             sem corpo de entrada.
  - manifesto.md -- sha256 das 3 fontes + do indice.md, no commit de referência.
...
Verificação embutida antes de escrever (aborta, nada é gravado):
  1. as fontes são exatamente as 3 e cada uma é filha direta da raiz do repo;
  2. REGRAS.md e PROJETO.md aparecem no indice.md como bloco verbatim contíguo;
  3. cada linha de título da Parte 3 é uma linha verbatim de MEMÓRIAS.md;
...
def linhas_titulo(mem_txt):
    """As linhas de título das entradas modernas, na ordem do arquivo
    (mais recente primeiro, desde MEMÓRIAS (271))."""
    ini = mem_txt.find(MARCADOR)
    if ini == -1:
        abortar("marcador ENTRADAS-NOVAS não achado em MEMÓRIAS.md.")
    corpo = mem_txt[mem_txt.find("\n", ini) + 1:]
...
def main():
    # 1. conjunto de entrada fixo, cada fonte filha direta da raiz do repo
    caminhos = {}
    for nome in FONTES:
    ...
    sha, data = carimbo()
    titulos = linhas_titulo(mem_txt)

    header = (
        "---\n"
        "gerado-de: canon público do sistema Agata (REGRAS.md, PROJETO.md, MEMÓRIAS.md)\n"
        f"canon: {sha}\n"
        f"data: {data}\n"
        ...
        "atual na íntegra, e a linha do tempo das entradas de memória (só os\n"
        f"títulos). Gerado do commit `{sha}` ({data}). Hashes em `manifesto.md`.\n"
    )
    parte3_cab = (
        "## PARTE 3 — MEMÓRIAS.md (linha do tempo, só títulos, mais recente primeiro)\n"
    ...
    for t in titulos:
        if t not in linhas_mem:
            abortar(f"linha de título não é verbatim de MEMÓRIAS.md: {t[:60]}")
    reconstruido = (
        header
        + SEP + "## PARTE 1 — REGRAS.md (íntegra)\n\n" + regras
        + SEP + "## PARTE 2 — PROJETO.md (íntegra)\n\n" + projeto
    ...
    manifesto = (
        ...
        "Conferir : sha256sum das 3 fontes bate com os valores acima.\n"
    )
