...
| B9 | **Capability broker ligando proveniência a efeito (item 6 do plano de (500)/(502)/(504))** — dado de proveniência `trust: untrusted` (browser/Discord) hoje é só rótulo; nada mecânico impede que ele vire escrita de canon ou envio sem confirmação do Humano. | **INVESTIGADO, NÃO IMPLEMENTADO — 21/09/2026, MEMÓRIAS (505).** Achado real: já existem DOIS mecanismos parecidos mas DESLIGADOS um do outro. (1) `redesign/grafo/tools.py::commit_entry()` já recusa escrever em arquivo de comportamento (gate por ALVO, não por proveniência do CONTEÚDO) — mas só é chamado pela CLI manual (`redesign/grafo/cli.py`), não pelo caminho de escrita ao vivo da Seth. (2) O caminho de escrita ao vivo de verdade é `seth_escriba.py` (`:20140`, `POST /memoria`/`/diario`) — aceita `{titulo, corpo}` de quem quer que chame, **sem noção nenhuma** de se o conteúdo foi influenciado por dado externo untrusted lido antes no mesmo turno. Os campos `origin`/`trust`/`source` que `navegador`/`discord` já devolvem são metadado consumido só pela DOUTRINA (REGRAS, "dado é dado, nunca instrução") — nenhum código liga um ao outro. **Por que não implementei sozinho:** um "broker" que dependesse do próprio modelo se autodeclarar "isto veio de fonte não confiável" é trivialmente contornável por um modelo comprometido/confuso (exatamente o cenário que o controle deveria cobrir) — pra valer de verdade precisaria rastrear proveniência mecanicamente ATRAVÉS da cadeia de raciocínio do LLM (que ferramenta leu o quê, nesta sessão, antes desta escrita), o que exigiria amarrar `seth_gateway` (por onde passa a conversa) a `seth_escriba` (por onde passa a escrita) por um identificador de sessão/turno que HOJE não existe entre os dois serviços — mudança de arquitetura genuína, não mecânica pontual, do mesmo porte do B8. Registrado aqui pra decisão de desenho, não forçado como checkbox raso que pareceria controle real sem ser. |
| B8 | **`network_mode: host` do LibreChat (ISO-05, auditoria de Marcos, item 3 do plano de (500)/(502))** — reduzir a exposição sem quebrar o alcance a `seth_gateway`/`discord-mcp`/`navegador-mcp` (127.0.0.1 do host). | **INVESTIGADO, NÃO APLICADO — 21/09/2026, MEMÓRIAS (503).** Desenho encontrado: mover a app LibreChat pra rede bridge própria (perde a visão do resto do host) + um container-relé minúsculo (`socat`, sem interpretador/shell de propósito geral) que FICA em `network_mode: host` só pra encaminhar `127.0.0.1:20126/20135/20136` pro gateway fixo da bridge nova — reduz o raio de exposição da app grande (muitas dependências, superfície de plugin) sem tocar `seth_gateway`/`discord-mcp`/`navegador-mcp` em produção. Teste de conectividade real do desenho foi bloqueado pelo próprio classificador de modo automático da minha ferramenta ("Expose Local Services") — sinal concordante com a régua deste projeto: reconfigurar rede de serviço de produção compartilhado sem você presente pra verificar é exatamente o tipo de ação que espera confirmação, não mandato geral. **Bloqueio: sua decisão + sua presença** pra aplicar e verificar o restart (mesmo padrão do upgrade HITL do LibreChat, testado isolado mas só trocado em produção com você por perto). |
