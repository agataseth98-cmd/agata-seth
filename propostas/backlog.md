...
| H4 | **Sem tier LOCAL de último recurso na cadeia da Seth** ((390)) | **PARCIAL — MEMÓRIAS (402).** `seth_local_shim` (`:20133`) construído, rodando, no P-9. Falta o registro no OmniRoute (provider + tier 5 do combo) — decisão pendente do Humano: B1 (2ª connection `ollama-local` pro shim), B2 (consertar a `ollama-local` existente, ~= opção a), ou B3 (repontar a `baseUrl` da `ollama-local` pro shim). |
