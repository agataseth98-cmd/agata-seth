...
| ~~B9~~ | ~~Capability broker ligando proveniência a efeito~~ | **FECHADO 23/09/2026 — MEMÓRIAS (532), decisão do Humano ("Fechar").** A trava mecânica que o B9 pedia existe agora nos dois clientes que escrevem: LibreChat com aprovação de ferramenta funcionando ((523)/(524), `ask: ["*"]`) e Goose com `shell`/`write`/`edit`/`memoria_acrescentar`/`diario_anotar` em `ask_before`. **Risco residual declarado:** um processo LOCAL comprometido ainda pode dar POST direto no `seth_escriba` (`:20140`, sem autenticação, só loopback). Reabre se a aprovação for desligada em qualquer um dos dois clientes. | — |
| B8 | **`network_mode: host` do LibreChat (ISO-05)** | **TENTADO E REVERTIDO 23/09/2026 — MEMÓRIAS (532), decisão do Humano ("Fazer agora").** Desenho completo testado: bridge `librechat` 172.29.7.0/24 + relé `librechat-ponte-host` (socat) nas **7** portas (20126, 8890, 27125, 20135, 20136, 20140, 20141 — a tentativa de 21/09 cobria só 4), Mongo/Meili pelo nome do serviço, HOST=0.0.0.0 só dentro do container. Resultado: isolamento funcionou, MAS as 7 portas também ficaram inalcançáveis — o `ufw` da Máquina bloqueia tráfego da bridge pro host. Revertido pro modo host (Seth de volta em ~6s). Config tentada em `logs/incidente-goose-librechat-2026-09-23/*.B8-tentativa*`. | Falta: (1) nome fixo da interface (`com.docker.network.bridge.name: br-librechat`); (2) regra `ufw` do Humano (sudo) liberando SÓ 172.29.7.0/24 → 172.29.7.1 nas 7 portas. |
