...
- **Candidato avaliado, não adotado: `qwen3.5:9b`** (nome parecido com `qwen3-14b-64k` acima, modelo diferente — checar sempre qual dos dois, ver MEMÓRIAS (114)). Testado (MEMÓRIAS (119)/(120)): tool-calling correto sob payload real com `num_ctx=65536`, mas **VRAM de pico maior que o fallback atual** (91,3% contra 83,8% dos 8.188 MiB da placa) — o argumento original de "libera VRAM" está refutado por medição; se adotado, é por offload/velocidade (30/33 camadas na GPU contra 23/41), não por folga. Um incidente de fabricação registrado sob contexto degradado (truncamento do teste, não do modelo em uso normal) — o `thinking` visível foi o que expôs. **Testado pelo caminho real de produção em (121), causa fechada em (133): não é bug do pipeline nem do modelo — é limitação documentada do Ollama.** O endpoint compatível com OpenAI (`/v1/chat/completions`, usado pelo `hermes-agent`) não honra `num_ctx` do pedido em nenhuma forma, confirmado pelo mantenedor do Ollama (issue `ollama/ollama#16814`, fechada). `qwen3-14b-64k`/`qwen2.5-14b-64k` funcionam porque têm `PARAMETER num_ctx 65536` embutido no próprio Modelfile, não porque o pedido é honrado. `qwen3.5:9b` (tag oficial) não tem esse parâmetro — adoção exige criar uma tag customizada com `num_ctx` embutido, mesmo padrão dos outros dois; sem isso, qualquer chamada pelo caminho de produção repete o corte. Não instalado como fallback. **Sem variante text-only na biblioteca oficial** (64 tags checadas, todas multimodais) — o encoder de visão é permanente nesta família de modelo, confirmado via `/api/show` (27 blocos de atenção de visão), medição isolada: 6.494 MiB de VRAM com `num_ctx=4096` só-texto (MEMÓRIAS (123)).
...
- **`num_ctx` ignorado pelo endpoint compatível com OpenAI do Ollama — fechado em MEMÓRIAS (133), não é bug do `hermes-agent`.** Achado em (121) com `qwen3.5:9b` (contexto cortado pra 4096 apesar de `context_length: 65536` configurado). A hipótese líder anterior (merge raso em `chat_completions.py:566-573`, (122)) foi testada e refutada por evidência direta: `HERMES_DUMP_REQUESTS=1` capturou o `api_kwargs` real antes do envio, com `options.num_ctx: 65536` presente — o pedido saiu certo. Causa real, confirmada por fonte primária: o mantenedor do Ollama declara, em `ollama/ollama#16814` (fechada), que o endpoint `/v1/chat/completions` não suporta `num_ctx` em nenhuma forma, por desenho. `qwen3-14b-64k`/`qwen2.5-14b-64k` nunca dependeram do pedido ser honrado — têm `PARAMETER num_ctx 65536` embutido no próprio Modelfile (`ollama show --modelfile`, confirmado nos dois). `qwen3.5:9b` não tem esse parâmetro — daí o corte. Reproduzido ao vivo em (133) pelo caminho manual (`hermes chat --provider "custom:qwen-local-ctx-override"`), sem tocar o fallback automático.
