...
**[AGUARDANDO ASSINATURA] Proposta nova: uma checagem automática que confere se as citações "arquivo, linha tal" que eu escrevo em MEMÓRIAS.md realmente batem com o arquivo de verdade.** Contei o catálogo de erros já registrado no sistema: 8 em cada 20 falhas conhecidas são exatamente isso — citar algo sem checar antes. Duas vezes essa mesma semana, incluindo uma vez em que EU MESMO disse que uma citação minha estava errada e não estava (corrigi na hora, publicamente). Construí a ferramenta, testei bastante (inclusive achei e consertei dois erros nela mesma, testando de verdade, não só descrevendo), e deixei pronta pra você assinar: `bash scripts/aprovar.sh p19-citacao-arquivo-2026-09-18`. Nada disso entrou no sistema ainda — só a proposta, esperando sua palavra.
