...
**[FALTA SÓ VOCÊ] Upgrade de verdade do LibreChat pronto — preciso da sua assinatura pra entrar.** Você disse "aplicar de verdade". Preparei os dois arquivos reais (troquei a versão da imagem, liguei o bloco de aprovação) — mas são arquivos que mudam comportamento do sistema, e a trava de segurança do projeto (P-8) não deixa eu mesmo aprovar isso, só registrar a proposta. **Rode**: `bash scripts/aprovar.sh librechat-hitl-upgrade-2026-09-21` (depois de dar uma olhada em `propostas/librechat-hitl-upgrade-2026-09-21.diff` se quiser). Depois disso eu aplico de verdade (troco o container em produção) e confirmo funcionando.
