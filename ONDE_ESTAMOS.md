...
**[BLOQUEADO, decisão sua] Você já instalou a extensão do navegador — mas o Goose ainda não consegue usá-la, porque ela está no navegador errado pro que a ferramenta espera.** Você voltou do reboot de (478) e clicou em "Adicionar ao Chrome". Conferi na Máquina: a extensão está lá, ativa, rodando de verdade dentro do Brave. O problema, achado testando de verdade: o programa que conecta o Goose ao navegador (`@playwright/mcp`) só sabe procurar extensão dentro do Google Chrome — Brave nunca é um lugar que ele olha, por padrão. Tentei corrigir apontando ele pro perfil certo do Brave, mas o próprio sistema de segurança desta sessão bloqueou — mexer no perfil de um navegador que já está aberto, com suas sessões reais (e-mail, Discord etc.), é risco demais pra eu decidir sozinho.
- **Três caminhos, escolha um:** (a) instalar o Google Chrome ou o Chromium nesta máquina e pôr a extensão lá — caminho que já funciona sem gambiarra; (b) fechar o Brave você mesmo e me deixar tentar apontar a ferramenta pro perfil dele, sem risco de brigar com o navegador aberto; (c) tentar a extensão reserva (`Browser MCP`, já configurada, desligada por padrão) em vez da Playwright — ainda não sei se ela tem o mesmo problema.
...
- Detalhe técnico completo: `MEMÓRIAS.md` (474)-(479).
